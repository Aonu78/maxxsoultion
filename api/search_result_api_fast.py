...
def ads(key):
    ...
    adl = []
    key = (key + " # # # # #").split()
    cursor = con.execute("SELECT * from ads_data WHERE keywords like '%"+key[0]+"%' or keywords like '%"+key[1]+ "%' or keywords like '%"+key[2]+"%' or keywords like '%"+key[3]+"%' or keywords like '%"+key[4]+"%'")
    c=1
    for row in cursor:
...
async def search(keyword):
              ...
              return {"results":l}
# for run the api uvicorn search_result_api_fast:app
